...
int main(int argc, const char* argv[]) {
  try {
    ...
    Session sess(url);
  {
    RowResultres = sess.sql("show variables like 'version'").execute();
  ...
  
  {
    ...
    add = coll.add(R"({"_id": "myuuid-1", "name": "foo", "age": 7})").execte();
  ...
  for (int i = 0; oc; ++i, doc = docs.fetchOne()) {
    cout << "doc#" << i << ":" << doc << endl;
